...
export const MealPlannerView: React.FC<MealPlannerViewProps> = ({
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-4 sm:px-6 py-4 space-y-6">
      ...
      <div className="bg-[#161822] p-2 rounded-2xl border border-[#252834]">
          ...
          {DAYS.map(day => {
            ...
            return (
              <button
                ...
                className={`flex-1 min-w-[72px] py-2.5 px-3 rounded-xl flex flex-col items-center gap-1 transition-all ${
                  isSelected
                    ? 'bg-[#FF5E3A] text-white font-bold shadow-lg shadow-[#FF5E3A]/30 scale-102'
                    : 'bg-[#1A1C24] hover:bg-[#252834] text-gray-400 hover:text-gray-200'
                }`}
              >
      ...
      <div className="bg-[#1A1C24] border border-[#252834] rounded-3xl p-5 space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Flame className="w-5 h-5 text-[#FF5E3A]" />
            <h3 className="text-sm font-bold text-white">{selectedDay}'s Daily Targets</h3>
          </div>
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-3 gap-2 pt-2 text-center text-xs">
            <div className="bg-[#161822] p-2 rounded-xl border border-[#252834]">
              ...
              <span className="font-bold text-white">{dailyTotals.protein}g / {userProfile.dailyProteinTarget}g</span>
            </div>
            <div className="bg-[#161822] p-2 rounded-xl border border-[#252834]">
              <span className="text-gray-400 block text-[10px]">CARBS</span>
              <span className="font-bold text-white">{dailyTotals.carbs}g</span>
            </div>
            <div className="bg-[#161822] p-2 rounded-xl border border-[#252834]">
              <span className="text-gray-400 block text-[10px]">HEALTHY FATS</span>
              <span className="font-bold text-white">{dailyTotals.fat}g</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {SLOTS.map(slot => {
          ...
          return (
            <div
              key={slot.id}
              className="bg-[#1A1C24] border border-[#252834] rounded-3xl p-4 sm:p-5 transition-all hover:border-[#2F3342]"
            >
              <div className="flex items-center justify-between mb-3">
                ...
                {!entry ? (
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#252834] hover:bg-[#FF5E3A] text-gray-200 hover:text-white text-xs font-bold transition-all shadow-sm active:scale-95"
                  >
              ...
              {recipe && entry ? (
                <div
                  ...
                  className="bg-[#161822] hover:bg-[#1E202B] border border-[#252834] rounded-2xl p-3 flex items-center justify-between gap-3 cursor-pointer transition-all group"
                >
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <div>
                      <h5 className="text-sm font-bold text-white group-hover:text-[#FF5E3A] transition-colors line-clamp-1">
                        {recipe.title}
                      </h5>
                ...
                <div 
                  onClick={() => setPickerSlot(slot.id)}
                  className="border border-dashed border-[#252834] hover:border-[#FF5E3A]/40 rounded-2xl p-4 text-center cursor-pointer transition-colors bg-black/10 hover:bg-white/5"
                >
                  <span className="text-xs text-gray-500 font-medium">No recipe planned for {slot.label.toLowerCase()}. Click to choose.</span>
                </div>
              )}
            </div>
          );
      ...
      {pickerSlot && (
        <div className="fixed inset-0 z-60 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#1A1C24] border border-[#252834] rounded-3xl p-6 max-w-md w-full space-y-4 shadow-2xl max-h-[85vh] flex flex-col">
            ...
            <input
              ...
              className="w-full bg-[#161822] text-sm text-white placeholder-gray-500 px-4 py-2.5 rounded-xl border border-[#252834] focus:outline-none focus:border-[#FF5E3A]"
            />
            ...
            <div className="flex-1 overflow-y-auto space-y-2 pr-1">
              {filteredPickerRecipes.map(r => (
                <div
                  ...
                  className="bg-[#161822] hover:bg-[#252834] border border-[#252834] rounded-2xl p-3 flex items-center justify-between cursor-pointer transition-all group"
                >
                  <div className="flex items-center gap-3">
                    <img
                      src={r.heroImage}
                    ...
                    />
                    <div>
                      <h5 className="text-sm font-bold text-white group-hover:text-[#FF5E3A] line-clamp-1">
                        {r.title}
                      </h5>
                      <span className="text-xs text-gray-400">{r.category} • {r.totalTimeMinutes}m • {r.calories} kcal</span>
                    </div>
                  </div>

                  <button className="px-3 py-1 rounded-full bg-[#FF5E3A] text-white text-xs font-bold shadow-sm">
                    Select
                  </button>
                </div>
              ))}
